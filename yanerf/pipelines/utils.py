...
def sample_grid(tensor: torch.Tensor, image_sampling_grid: torch.Tensor) -> torch.Tensor:
    ...
    assert image_sampling_grid[..., 0].max() < tensor_spatial_shape[-1], f"Invalid ray_sampler.image_width"
    assert image_sampling_grid[..., 1].max() < tensor_spatial_shape[0], f"Invalid ray_sampler.image_height"
