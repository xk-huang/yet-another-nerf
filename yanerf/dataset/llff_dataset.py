...
class LLFFDatasetWrapper(NamedTuple):
    poses: torch.Tensor
...
class LLFFDataset(Dataset):
    data_wrapper: Callable = LLFFDatasetWrapper
    ...
    def __init__(
        ...
        poses, bds, imgfiles = LLFFDataset._load_data(base_dir, factor=factor)
        ...
        if test_skip > 0:
            i_test = np.array([i for i in range(0, len(imgfiles), test_skip)])
        else:
            dists = np.sum(np.square(c2w[:3, 3] - poses[:, :3, 3]), -1)
            i_test = np.argmin(dists)
            i_test = np.array([i_test])
        logger.info(f"HOLDOUT view is/are:\n{i_test}")
        ...
        if split in ("val", "test"):
            poses = poses[i_test]
            imgfiles = imgfiles[i_test]
            bds = bds[i_test]
        else:
            i_train = np.array([i for i in range(len(imgfiles)) if i not in i_test])
            poses = poses[i_train]
            imgfiles = imgfiles[i_train]
            bds = bds[i_train]
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        file_path = self.imgfiles[index]

        pose = self.poses[index].astype(np.float32)
        h, w, focal = pose[:, -1]
        pose = pose[:, :4]
        min_depth, max_depth = self.bds[index].astype(np.float32)
        ...
        normalized_img = load_image(file_path)

        return (
            torch.from_numpy(pose),
            torch.FloatTensor([focal]),
            torch.from_numpy(normalized_img),
            torch.FloatTensor([min_depth]),
            torch.FloatTensor([max_depth]),
        )
    ...
    def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
        ...
        if factor is not None:
            sfx = f"_{factor}"
            LLFFDataset._minify(basedir, factors=[factor])
            factor = factor
        elif height is not None:
            factor = sh[0] / float(height)
            width = int(sh[1] / factor)
            LLFFDataset._minify(basedir, resolutions=[[height, width]])
        ...
        if not os.path.exists(imgdir):
            logger.info(f"{fimgdir} does not exist, returning")
            return
        ...
        if poses.shape[-1] != len(imgfiles):
            logger.info(f"Mismatch between imgs {len(imgfiles)} and poses {poses.shape[-1]} !!!!")
            return
